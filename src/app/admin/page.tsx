...
function LibraryStatusToggle({ adminEmail }: { adminEmail: string }) {
  ...
  if (!status) return <div style={{padding:"12px 14px"}}><p style={{fontSize:11,color:"rgba(255,255,255,.25)"}}>Loading…</p></div>;

  return (
    <div style={{padding:"14px",borderTop:"1px solid rgba(255,255,255,.06)",borderBottom:"1px solid rgba(255,255,255,.06)"}}>
      <p style={{fontSize:10,fontWeight:700,letterSpacing:".18em",textTransform:"uppercase",color:"rgba(255,255,255,.25)",marginBottom:10}}>Library Status</p>
      ...
      <div style={{marginBottom:12,paddingLeft:2}}>
        {status.is_open && status.opened_by && (
          <div>
            ...
            <p style={{fontSize:11,fontWeight:600,color:"rgba(255,255,255,.6)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:170}}>{status.opened_by.split("@")[0]}</p>
            <p style={{fontSize:10,color:"rgba(255,255,255,.25)",marginTop:1}}>{fmtDate(status.opened_at)} · {fmt12(status.opened_at)}</p>
          </div>
        )}
        {!status.is_open && status.closed_by && (
          <div>
            <p style={{fontSize:10,color:"rgba(255,255,255,.28)"}}>Closed by</p>
            <p style={{fontSize:11,fontWeight:600,color:"rgba(255,255,255,.6)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:170}}>{status.closed_by.split("@")[0]}</p>
            <p style={{fontSize:10,color:"rgba(255,255,255,.25)",marginTop:1}}>{fmtDate(status.closed_at)} · {fmt12(status.closed_at)}</p>
          </div>
        )}
      </div>
    ...
    <div style={{background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.08)",borderRadius:12,padding:"12px 14px",display:"flex",alignItems:"center",gap:12,minWidth:220}}>
      <div style={{width:32,height:32,borderRadius:"50%",background:"linear-gradient(135deg,#0f2040,#1E3A8A)",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:800,fontSize:12,flexShrink:0,overflow:"hidden"}}>
        {visit.students?.photo_url
          ? <Image src={visit.students.photo_url} alt={visit.students?.name||""} width={32} height={32} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
          : visit.students?.name?.charAt(0)||"?"
        }
      </div>
      <div style={{flex:1,minWidth:0}}>
        <p style={{fontSize:13,fontWeight:700,color:"#fff",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{visit.students?.name||"—"}</p>
        <p style={{fontSize:11,color:"rgba(255,255,255,.35)",marginTop:1}}>In at {visit.visit_time?.slice(0,5)}</p>
      </div>
      {confirm ? (
        <div style={{display:"flex",gap:5,flexShrink:0}}>
          ...
          <button onClick={()=>setConfirm(false)}
            style={{padding:"5px 10px",background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.15)",borderRadius:7,color:"rgba(255,255,255,.6)",fontSize:11,fontWeight:700,fontFamily:"'DM Sans',sans-serif",cursor:"pointer"}}>No</button>
        </div>
      ) : (
...
export default function AdminPage() {
  ...
  const TEXT_SIZE = textSize==="small"?13:textSize==="large"?16:14;
  const THEME = {
    bg:       darkMode?"#060d1a":"#f0f4f8",
    sidebar:  darkMode?"#0a1628":"#ffffff",
    main:     darkMode?"#060d1a":"#f0f4f8",
    card:     darkMode?"#0d1f3e":"#ffffff",
    border:   darkMode?"rgba(255,255,255,.07)":"rgba(0,0,0,.08)",
    text:     darkMode?"#ffffff":"#0f2040",
    textMuted:darkMode?"rgba(255,255,255,.45)":"rgba(15,32,64,.5)",
    sidebarBorder: darkMode?"rgba(212,175,55,.1)":"rgba(0,0,0,.08)",
  };

  return (
    <div style={{minHeight:"100vh",display:"flex",fontFamily:"'DM Sans',sans-serif",background:THEME.bg,color:THEME.text,fontSize:TEXT_SIZE}}>

      {/* ── SIDEBAR (desktop) ── */}
      <div className="admin-sidebar" style={{width:230,flexShrink:0,background:THEME.sidebar,borderRight:`1px solid ${THEME.sidebarBorder}`,display:"flex",flexDirection:"column",position:"sticky",top:0,height:"100vh"}}>
        {/* Logo */}
        ...
        <div style={{padding:"16px 12px",flex:1,overflowY:"auto"}}>
          <p style={{fontSize:10,fontWeight:700,letterSpacing:".2em",textTransform:"uppercase",color:"rgba(255,255,255,.25)",marginBottom:10,paddingLeft:6}}>Navigation</p>
          {NAV_ITEMS.map(item=>(
        ...
        <LibraryStatusToggle adminEmail={adminEmail}/>
      ...
      {/* ── MOBILE HEADER ── */}
      <div className="admin-mobile-header" style={{display:"none",position:"fixed",top:0,left:0,right:0,height:56,background:"#0a1628",borderBottom:"1px solid rgba(212,175,55,.1)",zIndex:100,alignItems:"center",justifyContent:"space-between",padding:"0 16px"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <p style={{fontSize:14,fontWeight:800,color:"#fff"}}>{NAV_ITEMS.find(n=>n.id===activePage)?.label||"Dashboard"}</p>
        </div>
        <button onClick={()=>setMobileNavOpen(!mobileNavOpen)}
          style={{background:"rgba(255,255,255,.07)",border:"1px solid rgba(255,255,255,.1)",borderRadius:8,padding:"8px 12px",color:"#fff",fontSize:16,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>
          ☰
        </button>
      </div>
        ...
        {mobileNavOpen && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            ...
            <motion.div initial={{x:-280}} animate={{x:0}} exit={{x:-280}} transition={{ease:EASE}}
              ...
              style={{width:280,height:"100%",background:"#0a1628",padding:"24px 16px",display:"flex",flexDirection:"column",gap:4}}>
              <p style={{fontSize:10,fontWeight:700,letterSpacing:".2em",textTransform:"uppercase",color:"rgba(255,255,255,.25)",marginBottom:10,paddingLeft:6}}>Navigation</p>
              {NAV_ITEMS.map(item=>(
                <button key={item.id} onClick={()=>{setActivePage(item.id);setMobileNavOpen(false);}}
                  style={{...sidebarBtnBase,background:activePage===item.id?"rgba(212,175,55,.08)":"transparent",color:activePage===item.id?"#DAA520":"rgba(255,255,255,.45)"}}>
      ...
      <div className="admin-main" style={{flex:1,overflow:"auto"}}>
        {activePage==="dashboard" && <DashboardPage visits={visits} loading={loading} today={today} weekAgo={weekAgo} monthStart={monthStart} yearStart={yearStart} cntToday={cntToday} cntWeek={cntWeek} cntMonth={cntMonth} cntYear={cntYear} onRefresh={fetchAll}/>}
        {activePage==="logs"      && <VisitorLogsPage visits={visits} loading={loading}/>}
        {activePage==="users"     && <UserManagementPage students={students} loading={loading} onRefresh={fetchAll}/>}
        {activePage==="schedule"  && <SchedulePage/>}
        {activePage==="help"      && <HelpManagementPage/>}
        {activePage==="settings"  && <SettingsPage darkMode={darkMode} textSize={textSize} onDarkMode={(v)=>{setDarkMode(v);localStorage.setItem("admin_dark_mode",String(v));}} onTextSize={(v)=>{setTextSize(v);localStorage.setItem("admin_text_size",v);}}/>}
      </div>
...
// DASHBOARD PAGE
// ══════════════════════════════════════════════════════════
function DashboardPage({ visits, loading, today, weekAgo, monthStart, yearStart, cntToday, cntWeek, cntMonth, cntYear, onRefresh }: {
  visits:Visit[]; loading:boolean; today:string; weekAgo:string; monthStart:string; yearStart:string;
  cntToday:number; cntWeek:number; cntMonth:number; cntYear:number; onRefresh:()=>void;
}) {
    ...
    <div style={{padding:"28px 32px",minHeight:"100vh"}}>
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:28,flexWrap:"wrap",gap:12}}>
        <div>
          ...
          <h1 style={{fontSize:32,fontWeight:900,color:"#fff",fontFamily:"'Playfair Display',serif",lineHeight:1.1}}>Visitor Dashboard</h1>
        </div>
   ...
   <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:14,marginBottom:24}}>
        {[
          ...
          <motion.div key={c.label} whileHover={{y:-2}} transition={{duration:.18}}
            style={{background:c.hi?"linear-gradient(145deg,#0d1f3e,#162d55)":"#0d1f3e",border:c.hi?"1px solid rgba(212,175,55,.25)":"1px solid rgba(255,255,255,.07)",borderTop:`2px solid ${c.hi?"#DAA520":"rgba(212,175,55,.25)"}`,borderRadius:12,padding:"20px 18px"}}>
            <p style={{fontSize:40,fontWeight:900,color:c.hi?"#DAA520":"#fff",fontFamily:"'Playfair Display',serif",lineHeight:1,marginBottom:8}}>{c.val}</p>
            <p style={{fontSize:13,fontWeight:700,color:c.hi?"rgba(212,175,55,.8)":"rgba(255,255,255,.65)",marginBottom:3}}>{c.label}</p>
            <p style={{fontSize:11,color:"rgba(255,255,255,.28)"}}>{c.sub}</p>
          </motion.div>
        ))}
      </div>

{/* Stat cards — Row 2 (clickable drill-down) */}
{(() => {
  // Unique today
  ...
  // Avg duration — only positive durations
  ...
  const row2Cards = [
    {
      ...
      color:"#93C5FD",
      drill: visits.filter(v=>v.visit_date===today)
        .filter((v,i,arr)=>arr.findIndex(x=>x.student_id===v.student_id)===i)
        .map(v=>({name:v.students?.name||"—",detail:v.students?.college||"—",time:v.visit_time?.slice(0,5)})),
      drillTitle:"Unique Visitors Today",
    },
    {
      ...
      sub:"Distinct students",
      color:"#6EE7B7",
      drill: visits.filter(v=>v.visit_date>=monthStart)
        .filter((v,i,arr)=>arr.findIndex(x=>x.student_id===v.student_id)===i)
        .map(v=>({name:v.students?.name||"—",detail:v.students?.college||"—",time:v.visit_date})),
      drillTitle:"Unique Visitors This Month",
    },
    {
      ...
      drill: visits
        .filter((v,i,arr)=>arr.findIndex(x=>x.student_id===v.student_id)===i)
        .map(v=>({name:v.students?.name||"—",detail:v.students?.college||"—",time:`${visits.filter(x=>x.student_id===v.student_id).length} visits`})),
      drillTitle:"All Time Unique Visitors",
    },
    {
      label:"Avg Visit Duration",
      val: avgStr,
      sub:`From ${positiveDurations.length} completed visits`,
      color:"#FCD34D",
      drill: done.filter((_,i)=>positiveDurations[i]>0).slice(0,20)
        .map(v=>{
          const[h1,m1,s1]=(v.visit_time||"00:00:00").split(":").map(Number);
          const[h2,m2,s2]=(v.time_out||"00:00:00").split(":").map(Number);
      ...
      drillTitle:"Completed Visit Durations",
    },
    {
      label:"Top Purpose",
      val: topPurpose,
      sub:`${reasonMap2[topPurpose]||0} visits`,
      color:"#F9A8D4",
      drill: visits.filter(v=>v.reason===topPurpose).slice(0,20)
        .map(v=>({name:v.students?.name||"—",detail:v.reason||"—",time:v.visit_date})),
      drillTitle:`Visitors — ${topPurpose}`,
    },
    {
      label:"Completion Rate",
      val: `${completionRate}%`,
      sub:"Checked out properly",
      color:"#4ade80",
      drill: visits.filter(v=>v.time_out).slice(0,20)
        .map(v=>({name:v.students?.name||"—",detail:v.visit_date,time:`${v.visit_time?.slice(0,5)} → ${v.time_out?.slice(0,5)}`})),
      drillTitle:"Completed Check-outs",
    },
  ];
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:14,marginBottom:24}}>
        {row2Cards.map(c=>(
          <motion.div key={c.label}
            whileHover={{y:-3,boxShadow:`0 8px 32px ${c.color}20`}}
            whileTap={{scale:.97}}
            transition={{duration:.18}}
            onClick={()=>setDrillDown({title:c.drillTitle,items:c.drill,color:c.color})}
            style={{background:"#0d1f3e",border:"1px solid rgba(255,255,255,.07)",borderTop:`2px solid ${c.color}`,borderRadius:12,padding:"18px 16px",cursor:"pointer",position:"relative",overflow:"hidden"}}>
            {/* Subtle glow bg */}
            ...
            <p style={{fontSize:12,fontWeight:700,color:"rgba(255,255,255,.7)",marginBottom:3}}>{c.label}</p>
            <p style={{fontSize:10,color:"rgba(255,255,255,.3)"}}>{c.sub}</p>
            {/* Click hint */}
        ...
        {drillDown && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            ...
            <motion.div initial={{opacity:0,scale:.92,y:24}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:.92,y:24}}
              ...
              onClick={e=>e.stopPropagation()}
              style={{width:"100%",maxWidth:560,background:"#0d1f3e",border:`1px solid ${drillDown.color}30`,borderTop:`2px solid ${drillDown.color}`,borderRadius:20,padding:"24px 24px",boxShadow:"0 24px 80px rgba(0,0,0,.6)",maxHeight:"75vh",display:"flex",flexDirection:"column"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18}}>
                <h3 style={{fontSize:18,fontWeight:900,color:"#fff",fontFamily:"'Playfair Display',serif"}}>{drillDown.title}</h3>
                <button onClick={()=>setDrillDown(null)}
                  style={{background:"rgba(255,255,255,.07)",border:"1px solid rgba(255,255,255,.1)",borderRadius:8,color:"rgba(255,255,255,.6)",fontSize:16,cursor:"pointer",width:34,height:34,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans',sans-serif"}}>✕</button>
              </div>
              <div style={{overflowY:"auto",display:"flex",flexDirection:"column",gap:8}}>
                {drillDown.items.length===0?(
                  <p style={{textAlign:"center",color:"rgba(255,255,255,.3)",padding:"40px 0",fontSize:14}}>No data available</p>
                ):drillDown.items.map((item,i)=>(
                  <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.06)",borderRadius:10}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <p style={{fontSize:13,fontWeight:700,color:"#fff",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{item.name}</p>
                      <p style={{fontSize:11,color:"rgba(255,255,255,.4)",marginTop:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{item.detail}</p>
                    </div>
              ...
              <div style={{flex:1,display:"flex",flexDirection:"column",gap:7}}>
                {collegeData.slice(0,6).map(([college,count],i)=>(
                  <div key={college} style={{display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <p style={{fontSize:11,color:"rgba(255,255,255,.55)",flex:1,lineHeight:1.3}}>{shortCollege(college)}</p>
          ...
          {dailyData.map(d=>(
            <div key={d.date} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
              ...
              <motion.div initial={{height:0}} animate={{height:`${Math.max((d.count/maxDaily)*80,d.count>0?6:3)}px`}} transition={{duration:.4,ease:EASE}}
                style={{width:"100%",borderRadius:"3px 3px 0 0",background:d.count>0?"linear-gradient(180deg,#DAA520,rgba(212,175,55,.4))":"rgba(255,255,255,.05)"}}/>
...
function VisitorLogsPage({ visits, loading }: { visits:Visit[]; loading:boolean }) {
  ...
  const selStyle:React.CSSProperties={height:40,padding:"0 14px",background:"#0d1f3e",border:"1px solid rgba(255,255,255,.15)",borderRadius:8,color:"rgba(255,255,255,.8)",fontSize:13,fontWeight:600,fontFamily:"'DM Sans',sans-serif",outline:"none",cursor:"pointer",colorScheme:"dark"};
  const dateStyle:React.CSSProperties={height:40,padding:"0 14px",background:"#0d1f3e",border:"1px solid rgba(255,255,255,.15)",borderRadius:8,color:"rgba(255,255,255,.8)",fontSize:13,fontFamily:"'DM Sans',sans-serif",outline:"none",cursor:"pointer",colorScheme:"dark"};
    ...
    <div style={{padding:"28px 32px",minHeight:"100vh"}}>
      ...
      <div style={{background:"#0d1f3e",border:"1px solid rgba(255,255,255,.07)",borderRadius:12,padding:"18px 20px",marginBottom:18}}>
        <div style={{position:"relative",marginBottom:14}}>
          ...
          <input type="text" placeholder="Search by name, email, student number, or college…" value={search} onChange={e=>setSearch(e.target.value)}
            style={{width:"100%",height:44,paddingLeft:42,paddingRight:search?40:18,background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.1)",borderRadius:9,color:"#fff",fontSize:14,fontFamily:"'DM Sans',sans-serif",outline:"none"}}
            onFocus={e=>e.target.style.borderColor="rgba(212,175,55,.45)"} onBlur={e=>e.target.style.borderColor="rgba(255,255,255,.1)"}/>
          {search&&<button onClick={()=>setSearch("")} style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"rgba(255,255,255,.4)",fontSize:16,fontFamily:"'DM Sans',sans-serif",padding:4}}>✕</button>}
        </div>
        ...
        <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <div><p style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,.3)",letterSpacing:".1em",textTransform:"uppercase",marginBottom:3}}>From</p><input type="date" value={dateFrom} onChange={e=>{setDateFrom(e.target.value);setPeriod("");}} style={dateStyle}/></div>
            <div style={{marginTop:16,color:"rgba(255,255,255,.3)",fontSize:14}}>→</div>
            <div><p style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,.3)",letterSpacing:".1em",textTransform:"uppercase",marginBottom:3}}>To</p><input type="date" value={dateTo} onChange={e=>{setDateTo(e.target.value);setPeriod("");}} style={dateStyle}/></div>
          </div>
          {(search||fReason||fCollege||dateFrom||dateTo||period!=="today")&&(
            <button onClick={clearFilters} style={{height:40,padding:"0 14px",background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.1)",borderRadius:8,color:"rgba(255,255,255,.5)",fontSize:12,fontWeight:700,fontFamily:"'DM Sans',sans-serif",cursor:"pointer"}}>Clear ✕</button>
          )}
      ...
      <div className="logs-mobile" style={{display:"none",flexDirection:"column",gap:10}}>
        {filtered.slice(0,50).map(v=>{
          ...
          return(
            <div key={v.visit_id} style={{background:"#0d1f3e",border:"1px solid rgba(255,255,255,.07)",borderRadius:12,padding:"14px 16px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                <div><p style={{fontWeight:700,color:"#fff",fontSize:14}}>{v.students?.name||"—"}</p><p style={{fontSize:11,color:"rgba(255,255,255,.35)",marginTop:2}}>{v.students?.email||v.student_id}</p></div>
              ...
              <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
                <div><p style={{fontSize:10,color:"rgba(255,255,255,.35)",marginBottom:2}}>Date</p><p style={{fontSize:12,fontWeight:600,color:"rgba(255,255,255,.7)"}}>{v.visit_date}</p></div>
                <div><p style={{fontSize:10,color:"rgba(255,255,255,.35)",marginBottom:2}}>Time In</p><span style={{background:"rgba(74,222,128,.08)",color:"#4ade80",padding:"3px 8px",borderRadius:4,fontSize:12,fontWeight:700}}>{v.visit_time?.slice(0,5)}</span></div>
                {v.time_out&&<div><p style={{fontSize:10,color:"rgba(255,255,255,.35)",marginBottom:2}}>Time Out</p><span style={{background:"rgba(248,113,113,.08)",color:"#f87171",padding:"3px 8px",borderRadius:4,fontSize:12,fontWeight:700}}>{v.time_out?.slice(0,5)}</span></div>}
              </div>
            </div>
          );
        })}
      ...
      {/* Desktop: table view */}
      <div className="logs-desktop" style={{background:"#0d1f3e",border:"1px solid rgba(255,255,255,.07)",borderRadius:12,overflow:"hidden"}}>
        {loading?<div style={{textAlign:"center",padding:"60px 20px",color:"rgba(255,255,255,.35)"}}>Loading records…</div>:(
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead>
                <tr style={{borderBottom:"1px solid rgba(255,255,255,.07)"}}>
                  {["Student","College","Purpose","Date","Time In","Time Out","Duration"].map(h=>(
                    <th key={h} style={{padding:"13px 20px",textAlign:"left",fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:".16em",color:"rgba(255,255,255,.28)",background:"rgba(255,255,255,.03)"}}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.length===0?(
                  <tr><td colSpan={7} style={{textAlign:"center",padding:"60px 20px"}}>
                    ...
                    <p style={{color:"rgba(255,255,255,.35)",fontSize:14,fontWeight:600}}>No records match your filters</p>
                  ...
                  return(
                    <tr key={v.visit_id} className="trow" style={{borderBottom:"1px solid rgba(255,255,255,.04)"}}>
                      <td style={{padding:"13px 20px"}}><p style={{fontWeight:700,color:"#fff",fontSize:14}}>{v.students?.name||"—"}</p><p style={{fontSize:11,color:"rgba(255,255,255,.35)",marginTop:2}}>{v.students?.email||v.student_id}</p></td>
                      <td style={{padding:"13px 20px",color:"rgba(255,255,255,.5)",fontSize:12}}>{shortCollege(v.students?.college||"—")}</td>
                      <td style={{padding:"13px 20px"}}>{v.reason?<span style={{background:rc.bg,color:rc.color,padding:"4px 10px",borderRadius:5,fontSize:12,fontWeight:700}}>{v.reason}</span>:<span style={{color:"rgba(255,255,255,.2)",fontStyle:"italic"}}>—</span>}</td>
                      ...
                      <td style={{padding:"13px 20px"}}>{v.time_out?<span style={{background:"rgba(248,113,113,.08)",color:"#f87171",padding:"4px 10px",borderRadius:5,fontSize:12,fontWeight:700}}>{v.time_out?.slice(0,5)}</span>:<span style={{color:v.visit_status==="inside"?"#FCD34D":"rgba(255,255,255,.2)",fontSize:12,fontStyle:v.visit_status==="inside"?"normal":"italic",fontWeight:v.visit_status==="inside"?700:400}}>{v.visit_status==="inside"?"● Inside":"—"}</span>}</td>
                      <td style={{padding:"13px 20px"}}>{(()=>{if(!v.time_out)return<span style={{color:"rgba(255,255,255,.2)",fontSize:12}}>—</span>;const[h1,m1]=v.visit_time.split(":").map(Number);const[h2,m2]=v.time_out.split(":").map(Number);const diff=(h2*60+m2)-(h1*60+m1);const hrs=Math.floor(diff/60);const mins=diff%60;return<span style={{fontSize:12,color:"rgba(255,255,255,.5)",fontWeight:600}}>{hrs>0?`${hrs}h `:""}{mins}m</span>;})()}</td>
                    </tr>
                  );
                })}
              </tbody>
...
// ══════════════════════════════════════════════════════════
function UserManagementPage({ students, loading, onRefresh }: { students:Student[]; loading:boolean; onRefresh:()=>void }) {
  const [search,        setSearch]        = useState("");
    ...
    <div style={{padding:"28px 32px",minHeight:"100vh"}}>
      ...
      <AnimatePresence>
        {editStudent && (
          <EditStudentModal
            ...
            onSaved={()=>{setEditStudent(null);onRefresh();}}
          />
        )}
      </AnimatePresence>
      ...
      <div style={{background:"#0d1f3e",border:"1px solid rgba(255,255,255,.07)",borderRadius:12,padding:"18px 20px",marginBottom:18}}>
        <div style={{position:"relative",marginBottom:14}}>
          ...
          <input type="text" placeholder="Search by name, email, student number, or college…" value={search} onChange={e=>setSearch(e.target.value)}
            style={{width:"100%",height:44,paddingLeft:42,background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.1)",borderRadius:9,color:"#fff",fontSize:14,fontFamily:"'DM Sans',sans-serif",outline:"none"}}
            onFocus={e=>e.target.style.borderColor="rgba(212,175,55,.45)"} onBlur={e=>e.target.style.borderColor="rgba(255,255,255,.1)"}/>
        </div>
      ...
      <div className="users-mobile" style={{display:"none",flexDirection:"column",gap:10}}>
        {filtered.map(s=>(
          <div key={s.student_id} style={{background:"#0d1f3e",border:"1px solid rgba(255,255,255,.07)",borderRadius:12,padding:"14px 16px",opacity:s.is_blocked?.7:1}}>
  ...
  {s.photo_url
    ? <Image src={s.photo_url} alt={s.name} width={36} height={36} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
    : s.name?.charAt(0)||"?"
  }
</div>
              <div style={{flex:1,minWidth:0}}>
                <p style={{fontWeight:700,color:s.is_blocked?"rgba(255,255,255,.5)":"#fff",fontSize:14,textDecoration:s.is_blocked?"line-through":"none",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.name}</p>
                <p style={{fontSize:11,color:"rgba(255,255,255,.35)",marginTop:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.email}</p>
              </div>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>
              <span style={{fontSize:11,color:"rgba(255,255,255,.5)",background:"rgba(255,255,255,.05)",padding:"3px 8px",borderRadius:5}}>{s.student_id}</span>
              <span style={{fontSize:11,color:"rgba(255,255,255,.5)",background:"rgba(255,255,255,.05)",padding:"3px 8px",borderRadius:5}}>{s.employee_status}</span>
            ...
            <p style={{fontSize:11,color:"rgba(255,255,255,.4)",marginBottom:10}}>{shortCollege(s.college)}</p>
            {s.programs&&<p style={{fontSize:11,color:"rgba(255,255,255,.35)",marginBottom:10}}>{s.programs.name}</p>}
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setEditStudent(s)} style={{flex:1,padding:"8px",background:"rgba(212,175,55,.1)",border:"1px solid rgba(212,175,55,.25)",borderRadius:8,color:"#DAA520",fontSize:12,fontWeight:700,fontFamily:"'DM Sans',sans-serif",cursor:"pointer"}}>✏️ Edit</button>
      ...
      {/* Desktop: table view */}
      <div className="users-desktop" style={{background:"#0d1f3e",border:"1px solid rgba(255,255,255,.07)",borderRadius:12,overflow:"hidden"}}>
        {loading?<div style={{textAlign:"center",padding:"60px 20px",color:"rgba(255,255,255,.35)"}}>Loading users…</div>:(
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead>
                <tr style={{borderBottom:"1px solid rgba(255,255,255,.07)",background:"rgba(255,255,255,.03)"}}>
                  {["Student","Student No.","College / Program","Type","Year","Status","Actions"].map(h=>(
                    <th key={h} style={{padding:"13px 20px",textAlign:"left",fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:".16em",color:"rgba(255,255,255,.28)"}}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.length===0?(
                  <tr><td colSpan={7} style={{textAlign:"center",padding:"60px 20px",color:"rgba(255,255,255,.28)",fontSize:14}}>No users match your filters</td></tr>
                ):filtered.map(s=>{
                  const isBlocked=s.is_blocked;
                  return(
                    <tr key={s.student_id} className="trow" style={{borderBottom:"1px solid rgba(255,255,255,.04)",opacity:isBlocked?.7:1}}>
  ...
  {s.photo_url
    ? <Image src={s.photo_url} alt={s.name} width={32} height={32} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
    : s.name?.charAt(0)||"?"
  }
</div>
                          <div>
                      ...
                      <td style={{padding:"13px 20px"}}>
                        <p style={{fontSize:12,color:"rgba(255,255,255,.5)"}}>{shortCollege(s.college)}</p>
                        {s.programs&&<p style={{fontSize:11,color:"rgba(255,255,255,.3)",marginTop:2}}>{s.programs.name}</p>}
                      </td>
                      <td style={{padding:"13px 20px"}}>
                      ...
                      <td style={{padding:"13px 20px"}}>
                        {s.year_level?<span style={{color:"#DAA520",fontSize:12,fontWeight:600}}>{YEAR_LABELS[s.year_level]}</span>:<span style={{color:"rgba(255,255,255,.2)",fontSize:12}}>—</span>}
                      </td>
                      <td style={{padding:"13px 20px"}}>
                      ...
                      <td style={{padding:"13px 20px"}}>
                        <div style={{display:"flex",gap:8}}>
                          <button onClick={()=>setEditStudent(s)}
                            ...
                            onMouseLeave={e=>(e.currentTarget.style.background="rgba(212,175,55,.08)")}>✏️ Edit</button>
                          <button onClick={()=>handleBlock(s)} disabled={blocking===s.student_id}
...
function EditStudentModal({ student, onClose, onSaved }: { student:Student; onClose:()=>void; onSaved:()=>void }) {
  ...
  const handleSave = async () => {
    ...
    const { error: err } = await supabase.from("students").update({
      ...
      program_id: selectedProgram?.id ?? student.program_id,
  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
      ...
      <motion.div initial={{opacity:0,scale:.92,y:24}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:.92,y:24}}
        ...
        style={{width:"100%",maxWidth:560,background:"#0d1f3e",border:"1px solid rgba(212,175,55,.2)",borderTop:"2px solid #DAA520",borderRadius:20,padding:"28px 28px",boxShadow:"0 24px 80px rgba(0,0,0,.6)",maxHeight:"90vh",overflowY:"auto"}}>
        ...
        {error&&<div style={{background:"rgba(220,38,38,.12)",border:"1px solid rgba(220,38,38,.28)",borderLeft:"3px solid #ef4444",borderRadius:11,padding:"10px 14px",marginBottom:16,fontSize:13,color:"#fca5a5"}}>⚠️ {error}</div>}
        {/* Photo upload/remove */}
        <div style={{marginBottom:16}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:16}}>
            <div style={{width:64,height:64,borderRadius:"50%",overflow:"hidden",border:"2px solid rgba(212,175,55,.35)",background:"linear-gradient(135deg,#0f2040,#1E3A8A)",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:800,fontSize:22,flexShrink:0}}>
              {photoPreview&&!removingPhoto
                ? <Image src={photoPreview} alt={name} width={64} height={64} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                : name.charAt(0)||"?"
              }
            </div>
      ...
      {[{value:1,label:"1st"},{value:2,label:"2nd"},{value:3,label:"3rd"},{value:4,label:"4th"},{value:5,label:"5th"}].map(y=>(
        ...
        <div style={{display:"flex",gap:10,marginTop:24}}>
          <button onClick={onClose}
            style={{flex:1,height:48,background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.12)",borderRadius:10,color:"rgba(255,255,255,.6)",fontSize:14,fontWeight:700,fontFamily:"'DM Sans',sans-serif",cursor:"pointer"}}>
            Cancel
          </button>
  ...
  return (
    <div style={{padding:"28px 32px",minHeight:"100vh"}}>
      ...
      <div style={{marginBottom:28}}>
        ...
        <h1 style={{fontSize:32,fontWeight:900,color:"#fff",fontFamily:"'Playfair Display',serif"}}>Library Schedule</h1>
      </div>

      {loading?<div style={{textAlign:"center",padding:"60px",color:"rgba(255,255,255,.35)"}}>Loading…</div>:(
        <>
          {/* Weekly schedule */}
          <div style={{background:"#0d1f3e",border:"1px solid rgba(255,255,255,.07)",borderRadius:12,overflow:"hidden",marginBottom:18}}>
            <div style={{padding:"18px 22px",borderBottom:"1px solid rgba(255,255,255,.07)"}}>
              <h3 style={{fontSize:16,fontWeight:800,color:"#fff"}}>Weekly Operating Hours</h3>
              <p style={{fontSize:13,color:"rgba(255,255,255,.4)",marginTop:4}}>Set the opening and closing times for each day of the week.</p>
            </div>
            <div style={{padding:"8px 0"}}>
              {schedules.map(sc=>(
                <div key={sc.id} style={{display:"flex",alignItems:"center",gap:16,padding:"14px 22px",borderBottom:"1px solid rgba(255,255,255,.04)",flexWrap:"wrap"}}>
                  <div style={{width:110,flexShrink:0}}>
                    <p style={{fontSize:14,fontWeight:700,color:"#fff"}}>{DAY_NAMES[sc.id]}</p>
                  </div>
                  {/* Is open toggle */}
                  ...
                  {sc.is_open&&(
                    <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                      <div>
                        <p style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,.35)",textTransform:"uppercase",letterSpacing:".1em",marginBottom:4}}>Opens</p>
                        <input type="time" defaultValue={sc.opening_time?.slice(0,5)} onBlur={e=>updateSchedule(sc.id,"opening_time",e.target.value+":00")}
                          style={{height:38,padding:"0 12px",background:"rgba(255,255,255,.07)",border:"1px solid rgba(255,255,255,.13)",borderRadius:8,color:"#fff",fontSize:13,fontFamily:"'DM Sans',sans-serif",outline:"none",colorScheme:"dark"}}/>
                      </div>
                      <span style={{color:"rgba(255,255,255,.3)",marginTop:16}}>→</span>
                      <div>
                        <p style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,.35)",textTransform:"uppercase",letterSpacing:".1em",marginBottom:4}}>Closes</p>
          ...
          {/* Special note */}
          <div style={{background:"#0d1f3e",border:"1px solid rgba(255,255,255,.07)",borderRadius:12,padding:"22px 24px"}}>
            <h3 style={{fontSize:16,fontWeight:800,color:"#fff",marginBottom:4}}>Special Note</h3>
            <p style={{fontSize:13,color:"rgba(255,255,255,.4)",marginBottom:16}}>This note appears on the closed modal (e.g. "Holiday today — No operations" or "Semestral Break").</p>
            <textarea value={scheduleNote} onChange={e=>setScheduleNote(e.target.value)} placeholder="Leave empty for no special note…"
              rows={3}
              style={{width:"100%",padding:"12px 16px",background:"rgba(255,255,255,.07)",border:"1.5px solid rgba(255,255,255,.13)",borderRadius:10,color:"#fff",fontSize:14,fontFamily:"'DM Sans',sans-serif",outline:"none",resize:"vertical",marginBottom:12}}
              onFocus={e=>e.target.style.borderColor="rgba(212,175,55,.55)"} onBlur={e=>e.target.style.borderColor="rgba(255,255,255,.13)"}/>
    ...
    <div style={{padding:"28px 32px",minHeight:"100vh"}}>
      ...
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:24,flexWrap:"wrap",gap:12}}>
        <div>
          ...
          <h1 style={{fontSize:32,fontWeight:900,color:"#fff",fontFamily:"'Playfair Display',serif"}}>Help Content</h1>
        </div>
          ...
          return(
            <button key={sec} onClick={()=>setActiveTab(sec)}
              style={{padding:"10px 20px",borderRadius:10,border:"1px solid",fontSize:13,fontWeight:700,fontFamily:"'DM Sans',sans-serif",cursor:"pointer",
                ...
                borderColor:activeTab===sec?meta.color:"rgba(255,255,255,.1)",
                color:activeTab===sec?meta.color:"rgba(255,255,255,.4)"}}>
      ...
      {loading?<div style={{textAlign:"center",padding:"60px",color:"rgba(255,255,255,.35)"}}>Loading…</div>:(
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {sectionItems.length===0?(
            <div style={{textAlign:"center",padding:"60px 20px",background:"#0d1f3e",border:"1px solid rgba(255,255,255,.07)",borderRadius:12}}>
              <p style={{fontSize:28,marginBottom:10}}>📭</p>
              <p style={{color:"rgba(255,255,255,.35)",fontSize:14}}>No items in this section yet.</p>
            ...
            <motion.div key={item.id} layout
              style={{background:"#0d1f3e",border:`1px solid ${item.is_active?"rgba(255,255,255,.07)":"rgba(255,255,255,.03)"}`,borderRadius:12,padding:"16px 20px",opacity:item.is_active?1:.55,display:"flex",alignItems:"flex-start",gap:14}}>
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                  <p style={{fontSize:14,fontWeight:700,color:"#fff"}}>{item.title}</p>
                  {!item.is_active&&<span style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,.3)",background:"rgba(255,255,255,.05)",padding:"2px 8px",borderRadius:4}}>HIDDEN</span>}
                </div>
                <p style={{fontSize:13,color:"rgba(255,255,255,.5)",lineHeight:1.6}}>{item.content}</p>
                <p style={{fontSize:11,color:"rgba(255,255,255,.25)",marginTop:6}}>Sort order: {item.sort_order}</p>
              </div>
              <div style={{display:"flex",gap:6,flexShrink:0}}>
                ...
                <button onClick={()=>deleteItem(item.id)}
                  style={{padding:"6px 12px",borderRadius:7,border:"1px solid rgba(255,255,255,.1)",fontSize:11,fontWeight:700,fontFamily:"'DM Sans',sans-serif",cursor:"pointer",background:"rgba(255,255,255,.04)",color:"rgba(255,255,255,.4)"}}>
                  Delete
                </button>
              </div>
            </motion.div>
...
function HelpItemModal({ item, defaultSection, onClose, onSaved }: {
  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
      ...
      <motion.div initial={{opacity:0,scale:.92,y:24}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:.92,y:24}}
        transition={{duration:.4,ease:EASE}}
        style={{width:"100%",maxWidth:520,background:"#0d1f3e",border:"1px solid rgba(212,175,55,.2)",borderTop:"2px solid #DAA520",borderRadius:20,padding:"28px 28px",boxShadow:"0 24px 80px rgba(0,0,0,.6)"}}>
        ...
        <div style={{display:"flex",gap:10,marginTop:22}}>
          <button onClick={onClose}
            style={{flex:1,height:46,background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.12)",borderRadius:10,color:"rgba(255,255,255,.6)",fontSize:14,fontWeight:700,fontFamily:"'DM Sans',sans-serif",cursor:"pointer"}}>
            Cancel
          </button>
...
// SETTINGS PAGE
// ══════════════════════════════════════════════════════════
function SettingsPage({ darkMode, textSize, onDarkMode, onTextSize }: {
  darkMode: boolean;
  ...
  return (
    <div style={{padding:"28px 32px",minHeight:"100vh"}}>
      <div style={{marginBottom:28}}>
        ...
        <h1 style={{fontSize:32,fontWeight:900,color:"#fff",fontFamily:"'Playfair Display',serif"}}>Settings</h1>
      </div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:16,maxWidth:580}}>
        ...
        {/* Theme */}
        <div style={{background:"#0d1f3e",border:"1px solid rgba(255,255,255,.07)",borderRadius:12,padding:"22px 24px"}}>
          <h3 style={{fontSize:16,fontWeight:800,color:"#fff",marginBottom:4}}>Appearance</h3>
          <p style={{fontSize:13,color:"rgba(255,255,255,.4)",marginBottom:20}}>Choose how the admin dashboard looks.</p>
          <div style={{display:"flex",gap:12}}>
            {[
              {val:true,  label:"Dark Mode",  icon:"🌙", desc:"Dark navy theme"},
              {val:false, label:"Light Mode", icon:"☀️", desc:"Clean white theme"},
            ].map(t=>(
              <button key={String(t.val)} type="button" onClick={()=>onDarkMode(t.val)}
                style={{flex:1,padding:"16px",borderRadius:12,border:"1.5px solid",cursor:"pointer",textAlign:"left",fontFamily:"'DM Sans',sans-serif",transition:"all .18s",
                  background:darkMode===t.val?"rgba(212,175,55,.1)":"rgba(255,255,255,.04)",
                  borderColor:darkMode===t.val?"#DAA520":"rgba(255,255,255,.1)",
                }}>
                <p style={{fontSize:22,marginBottom:8}}>{t.icon}</p>
                <p style={{fontSize:14,fontWeight:700,color:darkMode===t.val?"#DAA520":"rgba(255,255,255,.7)",marginBottom:3}}>{t.label}</p>
                <p style={{fontSize:12,color:"rgba(255,255,255,.35)"}}>{t.desc}</p>
        ...
        {/* Text Size */}
        <div style={{background:"#0d1f3e",border:"1px solid rgba(255,255,255,.07)",borderRadius:12,padding:"22px 24px"}}>
          <h3 style={{fontSize:16,fontWeight:800,color:"#fff",marginBottom:4}}>Text Size</h3>
          <p style={{fontSize:13,color:"rgba(255,255,255,.4)",marginBottom:20}}>Adjust the size of text across the dashboard.</p>
          <div style={{display:"flex",gap:10}}>
            {([
              {val:"small",  label:"Small",  size:13},
              {val:"medium", label:"Medium", size:14},
              ...
              <button key={t.val} type="button" onClick={()=>onTextSize(t.val)}
                style={{flex:1,padding:"16px 12px",borderRadius:12,border:"1.5px solid",cursor:"pointer",textAlign:"center",fontFamily:"'DM Sans',sans-serif",transition:"all .18s",
                  background:textSize===t.val?"rgba(147,197,253,.1)":"rgba(255,255,255,.04)",
                  borderColor:textSize===t.val?"#93C5FD":"rgba(255,255,255,.1)",
                }}>
                <p style={{fontSize:t.size,fontWeight:700,color:textSize===t.val?"#93C5FD":"rgba(255,255,255,.7)",marginBottom:4}}>Aa</p>
                <p style={{fontSize:12,fontWeight:700,color:textSize===t.val?"#93C5FD":"rgba(255,255,255,.5)"}}>{t.label}</p>
        ...
        {/* Info */}
        <div style={{background:"rgba(212,175,55,.06)",border:"1px solid rgba(212,175,55,.15)",borderRadius:12,padding:"16px 20px",display:"flex",gap:12,alignItems:"flex-start"}}>
